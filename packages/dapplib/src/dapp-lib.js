...
module.exports = class DappLib {
  ...
  //     {
  //       config: DappLib.getConfig(), //passes in dapp-config.json to know which account to read from and which transactions to un
  //       roles: {
  //         proposer: data.signer, //always gonna be signer/proposer/authorizer
  //       },
  //     },
  //     "kittyitems_mint_kitty_item", //will go to the kittyitems folder and call the transaction
  //     //transactions that takes parameters - create an object:
  //     {
  //       recipient: { value: data.recipient, type: t.Address }, //t.Address -> onFlow/types
  //       typeID: { value: parseInt(data.typeID), type: t.UInt64 }, //only for UInt64
  //     }
  //   );
  //   return {
  ...
  //       roles: {
  //         //no signer needed since its a script
  //       },
  //     },
  //     "kibble_get_balance",
  //     {
    ...
    let result = await Blockchain.post(
      {
        ...
        roles: {
          //NOTES:
          //why config.accounts[0]
          //kibble contract : init -> we store a minter resource to the account the kibble is deployed to
          //mint tokens -> signer always has to be that account that kibble is deployed to
          //config.accounts[0] = 0x01cf0e2f2f715450 -> the address the kibble contract is deployed to
